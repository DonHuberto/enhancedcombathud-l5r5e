...
HUD ma układ inspirowany interfejsem cRPG: duży portret i panel dynamicznych zasobów tworzą górny rząd wraz z pełnymi kwadratowymi kartami aktywnej broni i założonego pancerza. Fatigue i Strife są narastającymi torami zaczynającymi od zera; mogą przekraczać Endurance lub Composure, a licznik liczbowy nadal pokazuje pełne `wartość/próg`. Widok graficzny jest ograniczony do 24 kul w dwóch rzędach po 12. Void wykorzystuje obrócone kwadratowe znaczniki. Focus, Vigilance, Honor, Glory i Status tworzą jedną pionową listę z ikonami. Trzy szybkie zestawy I–III znajdują się bezpośrednio nad kartami wyposażenia. Ninjo i Giri znajdują się pod portretem, bezpośrednio nad pionową listą palet. Osobny dolny pas mieści ekonomię tury, pojedynczy rząd akcji i odseparowane zakończenie tury, a jego szerokość dopasowuje się do rzeczywiście zajętych elementów. Nad centrum znajdują się pierścienie z bieżącymi wartościami i wyróżnioną postawą. Hover lub fokus broni i pancerza otwiera pełną kartę informacyjną; hover broni przekazuje jawny zasięg do publicznego API Tactical Grid, jeśli moduł jest aktywny.

Umiejętności, Techniki i Ekwipunek są trzema osobnymi, przeszukiwalnymi paletami wyświetlanymi nad całym HUD-em. Ich przyciski mają nazwę po lewej oraz divider, karetę i ikonę przy prawej krawędzi. Zawartość każdej palety jest zwartą pionową listą kategorii i wpisów, a nie siatką dużych kafelków. Lewy klik techniki uruchamia jej rzut; jeżeli technika wymaga testu, ale nie narzuca skilla, Dice Picker pozwala wybrać kategorię. Prawy klik pozostaje jawną drogą do arkusza edycji. Długie opisy technik i akcji korzystają z dopasowanego do viewportu tooltipa z przewijanym wnętrzem. Poza konfliktem HUD zachowuje panel postaci i palety, ale nie pokazuje ekonomii tury ani konfliktowych akcji podstawowych. W konflikcie akcje są rozdzielane według metadanych core na „Wymaga rzutu” i „Bez rzutu”, lecz wszystkie przyciski znajdują się w jednym dolnym rzędzie. Action i Water Action są wyśrodkowane nad „Wymaga rzutu”, natomiast Movement nad „Bez rzutu”; licznik dodatkowej akcji Wody pojawia się wyłącznie w postawie Wody. `Wait` jest celowo ukryte, `Throw Item` pozostaje dostępne zgodnie z ustawieniem reguły domowej core, a `End Turn` jest oddzielone od pozostałych akcji.

Ikony w `icons/stats`, `icons/palettes` oraz `icons/economy` są wymienialnymi placeholderami SVG o `viewBox="0 0 32 32"`. Można podmienić ich treść bez zmiany nazw plików ani kodu adaptera.
