...
test("manifest targets Foundry 14 and the supported Core/System baselines", () => {
    ...
    assert.equal(manifest.relationships.systems[0].compatibility.minimum, "1.14.118");
    assert.equal(manifest.version, "2.0.15");
    assert.equal(manifest.manifest.endsWith("/releases/latest/download/module.json"), true);
    assert.equal(manifest.download.endsWith("/releases/download/v2.0.15/module.zip"), true);
